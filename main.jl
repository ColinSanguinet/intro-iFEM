...
#      If changes are made to the Muscade repo locally the use of Revise is recommended.
...
DirectSolver        = SweepX{2}     # 2 for Dynamic solver, 1 for Static solver
...
InvSolver        = DirectXUA{2,0,0}   # Dynamic solver for the inverse analysis
...
if bNodalForceSin
    ...
    [addelement!(model,DofLoad,[nodid[node]];field=:t2,value= NodalSin ) for node = 1:nnodes]
end
...
if bInverseAnalysis
    ...
    @functor with() costX(x, t, meas) = 100000 * (meas(t)-x)^2
    @functor with() costXother(x, t, meas) = 100 * (meas(t)-x)^2
    @functor with() costU(u, t) = 0.0005*(NodalSin(t)-u)^2
